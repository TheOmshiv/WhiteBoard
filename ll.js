...
function drawRubberbandShape(loc) {
   ...
   if (currentTool === "brush") {
        ...
        let radiusY = shapeBoundingBox.height / 4;
        ctx.beginPath();
        ctx.ellipse(mousedown.x, mousedown.y, radiusX, radiusY, Math.PI /4, 0, Math.PI * 2 , true);
        ctx.stroke();
    } 
...
function eraser() {
    ...
    ctx.arc(mousedown.x, mousedown.y, 8, 0, Math.PI * 2, false);
    ctx.fill();

}
// function eraser(){
//     context.globalCompositeOperation="destination-out";
//       context.arc(event.clientX -canvas.offsetLeft,event.clientY - canvas.offsetTop,8,0,Math.PI*2,false);
//       context.fill();
// }
function skribble(){
